    ...
    GraphShape graph_shape(const network::Reasoning& graph)
    {
        ...
        for (const auto& language : languages)
            shape.language_names.emplace_back(
                language, graph.get_name_of_node_size(language), graph.get_node_of_name_size(language));
        return shape;
    }
...
public:
    ...
    ~Impl()
    {
        ...
        if (_n) _n->clear_partial_query_session();
#endif
    }
    ...
    void init(io::OutputHandler output)
    {
        ...
        _command_executor = std::make_unique<CommandExecutor>(
            _n.get(), _script_engine.get(), _repl_state,
            [this](const std::string& line) { _interactive->process(line); });
        ...
        _script_engine->set_command_handler(
            [this](const std::vector<std::string>& cmd) { _command_executor->execute(cmd); });
    }
...
#ifndef __EMSCRIPTEN__
        _n->clear_partial_query_session();
#endif
    ...
    bool invoke_keyword_with_partial_policy(const std::string& keyword, const std::string& text, const bool force)
    {
        ...
        if (_repl_state->partial_load_mode)
        {
            ...
            if (!force && !keyword_text_complete(text)) return _script_engine->invoke_keyword(keyword, text, force);

            const auto assessment = partial_sparql::classify(text);
            const bool routed = _n->partial_query_active();
            if (!routed && !assessment.allowed_in_resident_slice)
                throw std::runtime_error(
                    "SPARQL resident-slice execution refused (" + std::string(partial_sparql::query_class_name(assessment.query_class))
                    + "): " + assessment.reason);
            if (routed && !assessment.allowed_with_routed_coverage)
                throw std::runtime_error(
                    "SPARQL routed execution refused (" + std::string(partial_sparql::query_class_name(assessment.query_class))
                    + "): " + assessment.reason);
            if (routed && !_n->partial_query_supports_layers(assessment.required_layers))
                throw std::runtime_error("SPARQL query requires representation layers absent from the routed manifest");
            if (routed && assessment.requires_complete_coverage && !_n->partial_query_certifiable())
                throw std::runtime_error(
                    "SPARQL operator requires a canonical manifest with exhaustive routing and verified shards: " + assessment.reason);

            _n->out("SPARQL execution metadata:", true);
            _n->out("  dataset_mode: partial", true);
            _n->out(std::string("  evaluation_scope: ") + (routed ? "routed_partial" : "resident_graph"), true);
            ...
            _n->out(std::string("  global_completeness: ") + (routed ? "pending" : "not_established"), true);

            if (routed)
                _n->begin_partial_query(text, assessment.required_layers,
                                        partial_sparql::runtime_contract(assessment.result_contract));

            const std::optional<GraphShape> before_shape = routed ? std::nullopt : std::optional<GraphShape>(graph_shape(*_n));
            const auto previous_cluster = _n->active_cluster_name();
            ...
            catch (...)
            {
                ...
                restore_cluster(_n.get(), previous_cluster);
                if (routed) _n->finish_partial_query(false, 0);
                if (before_shape && !(graph_shape(*_n) == *before_shape))
            ...
            if (dispatched)
            {
                ...
                if (routed)
                    _n->out("  completion_certificate: " + _n->finish_partial_query(true, 0), true);
                else
                {
...
console::Interactive::~Interactive() { delete _pImpl; }
...
std::string console::Interactive::get_version() { return network::Zelph::get_version(); }
bool console::Interactive::is_auto_run_active() const { return _pImpl->_repl_state->auto_run; }

bool console::Interactive::is_accumulating() const
{
    const auto& s = _pImpl->_repl_state;
    return s->accumulating_zelph || s->accumulating_inline_janet
        || s->accumulating_keyword || s->script_mode == ScriptMode::Janet;
}
    ...
    try
    {
        ...
        if (state->accumulating_keyword)
        {
            if (line.find_first_not_of(" \t\r") == std::string::npos)
            {
                ...
                if (dispatched)
                {
                    ...
                    state->keyword_buffer.clear();
                    state->keyword_prev_blank = false;
                    if (state->auto_run) _pImpl->_n->run(true, false, false, true);
                }
                else
                {
        ...
        if (trimmed_utf8 == "%")
        {
            ...
            if (state->script_mode == ScriptMode::Janet)
            {
                ...
                else state->script_mode = ScriptMode::Zelph;
            }
            else state->script_mode = ScriptMode::Janet;
            return;
        }
        ...
        if (trimmed_utf8[0] == '%')
        {
            ...
            if (state->partial_load_mode && !state->partial_language_extension_import)
                throw std::runtime_error("Raw Janet input is blocked while a partial graph is loaded");
#endif
            std::string janet_code = zelph::string::trim_left(trimmed_utf8.substr(1));
        ...
        if (state->accumulating_zelph) state->zelph_buffer += "\n" + line;
        else state->zelph_buffer = line;
        ...
        else if (complete_stmt.find_first_not_of(" \t\n") != std::string::npos)
...
void console::Interactive::import_file(const std::string& file) const { _pImpl->import_with_partial_policy(file, {}); }
...
std::string console::Interactive::get_lang() const { return _pImpl->_n->get_lang(); }
void console::Interactive::set_output_handler(io::OutputHandler output) const { _pImpl->_n->set_output_handler(std::move(output)); }
void console::Interactive::out(const std::string& text, bool newline) const { _pImpl->_n->emit(io::OutputChannel::Out, text, newline); }
void console::Interactive::err(const std::string& text, bool newline) const { _pImpl->_n->emit(io::OutputChannel::Error, text, newline); }
void console::Interactive::log(const std::string& text, bool newline) const { _pImpl->_n->emit(io::OutputChannel::Diagnostic, text, newline); }
void console::Interactive::prompt(const std::string& text, bool newline) const { _pImpl->_n->emit(io::OutputChannel::Prompt, text, newline); }

#ifdef PROVIDE_C_INTERFACE
console::Interactive interactive;
extern "C" void zelph_process_c(const char* line, size_t len)
{
    if (len > 0) interactive.process(std::string(line, 0, len));
}
extern "C" void zelph_run() { interactive.run(true, false, false); }
#endif
